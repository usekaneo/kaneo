...
<Warning>
  Kaneo's better-auth upgrade to 1.7 changed the callback path from `/api/auth/oauth2/callback/custom` to `/api/auth/callback/custom`. If you configured a custom OAuth provider on an earlier version, update the redirect URI registered with your identity provider to match, or sign-in will fail.
</Warning>
...
3. Set the redirect URI to: `{KANEO_API_URL}/api/auth/callback/custom`
   - Example: `https://api.kaneo.example.com/api/auth/callback/custom`
...
```
{KANEO_API_URL}/api/auth/callback/custom
```

For example, if `KANEO_API_URL=https://api.kaneo.example.com`, the redirect URI should be:

```
https://api.kaneo.example.com/api/auth/callback/custom
```
