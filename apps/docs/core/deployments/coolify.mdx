...
5. **Set environment variables** (pre-filled from the compose file):
   ...
   - `KANEO_IMAGE_TAG` (optional): the Kaneo image version to deploy. It defaults to `latest`; set it to a release tag such as `2.28.3` to hold the deployment on a known version.
6. **Deploy**.
