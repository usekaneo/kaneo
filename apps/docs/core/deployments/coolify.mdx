...
5. **Set environment variables** (pre-filled from the compose file):
   ...
   - `KANEO_IMAGE_TAG` (optional): the Kaneo image version to deploy. It defaults to a pinned release tag; set it only to upgrade to a specific version.
6. **Deploy**.
...
- **Pinned Kaneo version**: the image is pinned to a specific release tag (not `latest`) so redeploys are deterministic. Upgrade by setting `KANEO_IMAGE_TAG` to a newer release tag.
