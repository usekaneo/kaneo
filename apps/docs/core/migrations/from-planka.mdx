...
- A PLANKA account that can see the boards you want to move.
...
- **Archive and trash lists.** Skipped deliberately; only active and closed
  lists are migrated.
- **Comment authorship.** Kaneo attributes imported comments to the owner of the
  API key, so the original author and date are preserved in the comment body.
...
**Two-factor authentication** — password login can't complete a TOTP challenge.
Copy an access token from your browser's dev tools and pass `--planka-token`
