...
### After (current recommended configuration)
...
Remove the `api` and `web` service blocks and add the `kaneo` service. The current recommended `postgres` configuration adds a healthcheck and keeps the database on the internal Compose network without publishing port 5432. Preserve the actual database name, user and password from your existing installation in `.env`; defaults do not rename an existing database.
