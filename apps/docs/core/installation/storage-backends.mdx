...
description: Configure image uploads with MinIO, AWS S3, or Cloudflare R2.
---

## Overview

Kaneo uses S3-compatible object storage for image uploads in:
...
- `S3_PUBLIC_BASE_URL` is recommended when your asset URL should be different from the raw S3 endpoint.
...
S3_PUBLIC_BASE_URL=http://localhost:9000/kaneo-uploads
```
...
S3_REGION=us-east-1
S3_FORCE_PATH_STYLE=false
S3_PUBLIC_BASE_URL=https://kaneo-uploads.s3.us-east-1.amazonaws.com
```

For another AWS region, adjust both:
...
- `S3_PUBLIC_BASE_URL` should usually point to your public bucket domain or custom domain
- if your bucket is not public, image rendering will not work from stored asset URLs
...
- check that `S3_PUBLIC_BASE_URL` points to the correct public asset host
