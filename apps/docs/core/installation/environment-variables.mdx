...
| `REGISTRATION_URL_UUID` | Protect email/password registration behind a private URL. When set, the public `/auth/sign-up` page redirects to sign-in, the sign-up link is hidden, and account creation is allowed only from `/auth/register/<your-uuid>`. | unset |
