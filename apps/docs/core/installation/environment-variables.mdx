...
Kaneo supports Redis for horizontal scaling of WebSocket broadcasts. When `REDIS_URL` is set, multiple API instances can relay real-time updates via Redis Pub/Sub. When not set, an in-memory adapter is used (single-instance only).

Name | Description |
--- | --- |
| `REDIS_URL` | Redis connection string (e.g., `redis://localhost:6379`). Enables multi-instance WebSocket broadcasts via Redis Pub/Sub.
