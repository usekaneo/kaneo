...
Kaneo uses S3-compatible object storage for image uploads in task descriptions and task comments.
...
| `S3_BUCKET` | The bucket Kaneo will use for uploaded images. |  |
...
| `S3_PUBLIC_BASE_URL` | Optional public base URL for uploaded assets. Use this when objects should be served from a public hostname or reverse proxy instead of the raw API endpoint. |  |
| `S3_FORCE_PATH_STYLE` | Use path-style S3 URLs. This should usually be `true` for MinIO and `fs`. | `true` |
| `S3_MAX_IMAGE_UPLOAD_BYTES` | Maximum allowed image upload size in bytes. | `10485760` |
