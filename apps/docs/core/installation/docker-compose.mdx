...
Kaneo uses S3-compatible object storage for image uploads in task descriptions and comments.

- Object storage is optional. Kaneo runs without it, but image uploads in task descriptions and comments will be unavailable.
- For local/self-hosted setups, MinIO is the recommended option when you want uploads.
- The Kaneo API creates presigned upload URLs and the browser uploads images directly to the configured storage backend.
...
Add this service and volume to your `compose.yml` only if you want image uploads:
