...
Kaneo can post task activity to a Slack channel whenever events occur in a project. This uses Slack's incoming webhook feature, so no bot token or OAuth flow is required.
