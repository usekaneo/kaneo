...
Kaneo ships an official MCP server package, `@kaneo/mcp`, so you can manage workspaces, projects, tasks, comments, and labels from MCP-compatible tools.

The server runs locally over stdio and authenticates with Kaneo using device authorization.
