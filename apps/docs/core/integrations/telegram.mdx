...
- For a personal chat: start a conversation with the bot and send any message.
- For a group or supergroup: add the bot, then send a message in that chat.
- For a channel: add the bot as an administrator if needed so it can post updates.
- For forum-style supergroups: add the bot to the group and note the topic you want to post into.
