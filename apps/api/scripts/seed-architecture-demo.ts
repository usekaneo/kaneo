/**
 * Seed do workspace "Arquitetura Demo" (~200 tasks) para pitch/demo.
 *
...
const OWNER_EMAILS = [
  "contato@elsesystems.com",
  "samuelelse2@gmail.com",
] as const;
...
const GITHUB_EXEMPT = new Set(["contract", "contrato", "reuniao", "meeting"]);
...
/** 6 projetos × colunas tipadas ≈ 200 tasks. */
const PROJECTS: ProjectSeed[] = [
  {
    type: "development",
    name: "Residencial Vista Mar — BIM",
    slug: "residencial-vista-mar-bim",
    icon: "Home",
    description:
      "Projeto executivo residencial alto padrão (RVT/IFC), compatibilização e entregáveis AS-BUILT.",
    taskCount: 40,
  },
  {
    type: "maintenance",
    ...
    icon: "Wrench",
    description:
      "Retrofit estrutural e de fachada — patologias, laudos e correções em obra ocupada.",
    taskCount: 35,
  },
  {
    type: "support",
    name: "Acompanhamento de Obra — Comercial Moema",
    slug: "acompanhamento-obra-moema",
    icon: "Headphones",
    description:
      "Suporte ao canteiro: RFIs, medições, dúvidas de detalhamento e alinhamento com construtora.",
    taskCount: 32,
  },
  {
    type: "hr",
    name: "People — Studio Arquitetura",
    slug: "people-studio-arquitetura",
    icon: "Users",
    description:
      "Recrutamento de arquitetos, estagiários e coordenadores de projeto.",
    taskCount: 28,
  },
  {
    type: "marketing",
    name: "Marca & Portfólio 2026",
    slug: "marca-portfolio-2026",
    icon: "Megaphone",
    description:
      "Campanhas, conteúdo e materiais do escritório para captação de clientes corporativos.",
    taskCount: 30,
  },
  {
    type: "operations",
    name: "Implantação & Obra — Interiores Jardins",
    slug: "implantacao-interiores-jardins",
    icon: "Settings",
    description:
      "Implantação de interiores, consultoria de layout e treinamento de facilities.",
    taskCount: 35,
  },
];
  ...
  try {
    ...
    for (let p = 0; p < PROJECTS.length; p++) {
      ...
      for (let t = 0; t < project.taskCount; t++) {
        ...
        const column = columns[t % columns.length]!;
        const number = t + 1;
        ...
        if (taskType === "contract" && contractTotal < 4) {
