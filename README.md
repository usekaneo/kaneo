...
Kaneo supports multiple authentication methods:

**Email/Password** - Built-in authentication (enabled by default)
...
**OIDC/OpenID Connect** - Enterprise SSO support (optional)
| Variable | Description | Example |
| -------- | ----------- | ------- |
| `OIDC_CLIENT_ID` | Your OIDC client ID | `kaneo-app` |
| `OIDC_CLIENT_SECRET` | Your OIDC client secret | Generated by your provider |
| `OIDC_DISCOVERY_URL` | OIDC provider discovery endpoint | `https://auth.example.com/.well-known/openid-configuration` |
| `OIDC_SCOPES` | Requested OAuth scopes | `openid profile email` |

**Setting up OIDC:**
1. Configure your OIDC provider (Keycloak, Auth0, Okta, etc.)
2. Set the redirect URI to: `http://your-domain:1337/api/auth/callback/oidc`
3. Add the environment variables to your `.env` file or Docker environment
4. Restart the API server

> **Note:** The OIDC sign-in button will appear on the login page when `OIDC_DISCOVERY_URL` is configured.
